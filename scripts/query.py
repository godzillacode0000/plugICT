...
import sys, os, io, tarfile, struct, sqlite3, tempfile, shutil, atexit, argparse, signal, time, logging
logging.basicConfig(level=logging.WARNING, format='[%(levelname)s] %(message)s')
log = logging.getLogger('ict_vault')
...
def _cleanup_temp():
    for d in _temp_dirs:
        try:
            ...
            log.warning("Temp cleanup warning: %s", e)

atexit.register(_cleanup_temp)

# Signal handlers for crash/temp cleanup
...
def check_cache(key):
    try:
        ...
        pass  # Non-critical: cache DB may be locked by another process
    return None
...
def rerank(query, candidates, top_k=5):
    ...
    try:
        ...
        log.warning("Cross-encoder load failed: %s", e)
...
def load_vault():
    ...
    try:
        ...
        log.info("Failed to unlock vault: %s", e)
        print("ERROR: Cannot unlock vault. License key invalid or file corrupted.")
        sys.exit(1)
    ...
    try:
        ...
        print("ERROR: Cannot decrypt vault. Please check your license key.")
        sys.exit(1)
...
def search(args):
    ...
    # Glossary — no vault decrypt needed (uses ICT_SHORTFORMS dict)
    if args.glossary:
    ...
    try:
        ...
        log.warning("FTS5 search unavailable: %s", e)
    ...
    try:
        ...
        log.warning("ChromaDB search unavailable: %s", e)
    ...
    for r in ranked:
        ...
        # Convert timestamp to seconds for YouTube deep link
        ts_str = r['start_ts'] or '0:00'
        ...
        line += f"   {r['text'][:300]}...\n"
        if r.get('video_id'):
            line += f"   🔗 https://youtu.be/{r['video_id']}?t={ts_seconds}\n"
        line += ""
